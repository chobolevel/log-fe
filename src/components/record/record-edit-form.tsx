...
export function RecordEditForm({ id }: RecordEditFormProps) {
  ...
  const {
    ...
    values: record
      ? {
          ...
          intensity: record.emotion?.intensity,
        }
      : undefined,
  });
  ...
  if (isLoading) return <RecordEditFormSkeleton />;
...
function RecordEditFormSkeleton() {
  return (
